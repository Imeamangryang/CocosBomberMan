...
# 폭탄 개수 증가 아이템
class BombItem(cocos.sprite.Sprite):
    def __init__(self, x, y):
        super(BombItem, self).__init__('item.png', position = (x, y))
...
# 폭탄 범위 증가 아이템
class Bombpowder(cocos.sprite.Sprite):
    def __init__(self, x, y):
        super(Bombpowder, self).__init__('item2.png', position = (x, y))
        self.position = eu.Vector2((x * 32) + 16, (y * 32) + 16)
        self.cshape = cm.AARectShape(self.position, self.width * 0.1, self.height * 0.1)
        self.posx = x
        self.posy = y
...
# 이동 속도 증가 아이템
class Moveincrease(cocos.sprite.Sprite):
    def __init__(self, x, y):
        super(Moveincrease, self).__init__('item3.png', position = (x, y))
        self.position = eu.Vector2((x * 32) + 16, (y * 32) + 16)
        self.cshape = cm.AARectShape(self.position, self.width * 0.1, self.height * 0.1)
        self.posx = x
        self.posy = y
